...
import { createClientMessageId, MY_USER_ID, MOCK_ROOM_ID } from "../api/mock/db";
...
export default function ChatRoomPage() {
  ...
  const roomId = user?.roomId ?? MOCK_ROOM_ID;
  ...
  }, [hasMore, nextBeforeMessageId, showToast]);
    ...
    (async () => {
      try {
        const [partnerInfo, page, myBookmarkedMessageIds, allReactions, partnerReadState] =
          await Promise.all([
            fetchChatPartner(),
            fetchMessages({ beforeMessageId: null }),
            fetchBookmarkedMessageIds(),
            fetchReactions(),
            fetchPartnerReadState(),
          ]);
        if (cancelled) return;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const savedMessage = await sendMessage({ content, clientMessageId });
      setMessages((prev) =>
