...
-- 포함 지표: 개인/연인 감정 흐름, 주요 감정, 자주 쓰는 단어, 메시지/이미지/공감 수,
-- 평균 응답 시간, 가장 활발한 시간, 날짜별 대화량, 북마크, 읽음 상태, 오늘의 기분.
-- 마지막에 기존 대시보드 스냅샷을 제거하므로 다음 조회 시 이 원본으로 다시 집계됩니다.
...
SELECT
    @room_id,
    CASE
        WHEN MOD(report_message.slot_number, 2) = 1 THEN @test1_id
        ELSE @test2_id
    END,
    ...
    CASE report_message.slot_number
        ...
        END
        WHEN 3 THEN CASE MOD(report_message.day_offset, 4)
            WHEN 0 THEN '오늘 저녁에 맛있는 거도 같이 먹자'
            WHEN 1 THEN '힘든 일 있으면 언제든지 나한테 말해'
            WHEN 2 THEN '점심도 거르지 말고 꼭 챙겨 먹어'
            ELSE '그럼 저녁에 만나서 같이 산책하자'
        END
        WHEN 4 THEN CASE MOD(report_message.day_offset, 4)
            WHEN 0 THEN '좋아 저녁에 뭐 먹을지 같이 고르자'
            WHEN 1 THEN '응 고마워 너한테 이야기하면 마음이 편해'
            WHEN 2 THEN '응 알겠어 너도 점심 꼭 챙겨 먹어'
            ELSE '좋아 저녁에 만나서 천천히 걷자'
        END
        WHEN 5 THEN CASE MOD(report_message.day_offset, 5)
        ...
        END
        WHEN 6 THEN CASE MOD(report_message.day_offset, 5)
        ...
        END
        WHEN 7 THEN CASE MOD(report_message.day_offset, 5)
            WHEN 0 THEN '그럼 다음에는 한강에도 같이 가자'
            WHEN 1 THEN '좋아 다음에는 네가 보고 싶은 걸로 보자'
            WHEN 2 THEN '위로해 줘서 고마워 마음이 편해졌어'
            WHEN 3 THEN '숙소랑 가고 싶은 곳을 몇 군데 찾아볼게'
            ELSE '오늘 같이 얘기도 많이 해서 더 좋았어'
        END
        ELSE CASE MOD(report_message.day_offset, 5)
            WHEN 0 THEN '좋아 다음 데이트도 진짜 기대된다'
            WHEN 1 THEN '응 좋아 오늘도 같이 보내서 행복했어'
            WHEN 2 THEN '오늘은 일찍 쉬고 내일은 더 좋은 하루 보내자'
            WHEN 3 THEN '응 우리 같이 계획하면 재미있을 것 같아'
            ELSE '나도 좋았어 오늘도 잘 자고 내일 보자'
        END
    END,
...
JOIN chat_message message
 ...
 AND message.sender_id = CASE
      WHEN MOD(report_id.slot_number, 2) = 1 THEN @test1_id
      ELSE @test2_id
 END
ON DUPLICATE KEY UPDATE
    emotion_type = VALUES(emotion_type),
...
-- 두 테스트 계정 모두 가장 최근 메시지까지 읽은 상태로 맞춥니다.
...
-- 대시보드는 원본 데이터로부터 계산되는 파생 데이터입니다.
-- 이전 집계가 남아 있으면 새 더미데이터가 보이지 않을 수 있으므로 다음 조회 전에 제거합니다.
DELETE FROM member_dashboard
WHERE room_id = @room_id;

DELETE FROM couple_dashboard
WHERE room_id = @room_id;
...
-- 감정 흐름과 주요 감정에 사용할 완료 분석 결과를 확인합니다.
SELECT
    message.sender_id,
...
JOIN chat_message message
  ON message.message_id = analysis.message_id
WHERE message.room_id = @room_id
  AND analysis.analysis_status = 'COMPLETED'
GROUP BY message.sender_id, analysis.emotion_type
ORDER BY message.sender_id, emotion_count DESC, analysis.emotion_type;

-- 커플/개인 대시보드의 정량 지표 원본이 모두 준비됐는지 확인합니다.
SELECT
    (SELECT COUNT(*)
     FROM chat_message
     WHERE room_id = @room_id) AS total_message_count,
    (SELECT COUNT(*)
    ...
    (SELECT COUNT(*)
     FROM chat_reaction
     WHERE room_id = @room_id) AS total_reaction_count,
    (SELECT COUNT(*)
     FROM chat_bookmark
     WHERE room_id = @room_id) AS total_bookmark_count,
    (SELECT COUNT(*)
     FROM chat_analysis analysis
    ...
    (SELECT COUNT(*)
     FROM mood
     WHERE room_id = @room_id) AS total_mood_count;

-- 두 스냅샷 결과가 0이면 정상입니다. 다음 대시보드 API 조회에서 새로 생성됩니다.
SELECT
    (SELECT COUNT(*)
     FROM member_dashboard
     WHERE room_id = @room_id) AS remaining_member_snapshots,
    (SELECT COUNT(*)
     FROM couple_dashboard
     WHERE room_id = @room_id) AS remaining_couple_snapshots;
